...
const ACTION_API_KEY = process.env.ACTION_API_KEY || "MySecretKey123456789";
...
app.use((req, res, next) => {
  ...
  if (key !== ACTION_API_KEY) return res.status(401).json({ error: "Unauthorized" });
  next();
});
...
function targetSize(aspect) {
  ...
  return { w: 1920, h: 1080 }; // по умолчанию 16:9
}
...
app.get("/status", async (req, res) => {
  try {
    ...
    if (b.status !== "completed") return res.json({ status: b.status });
    ...
    for (const line of text.split("\n").filter(Boolean)) {
      ...
      if (!b64) continue;
      const cropped = await toAspect(Buffer.from(b64, "base64"), "16:9");
